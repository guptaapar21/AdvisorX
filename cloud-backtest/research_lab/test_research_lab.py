...
def test_feature_cutoff_ignores_future_mutation():
    ...
    c.loc[c.index > t, ['close', 'volume']] = 9999
    b = feature_snapshot(c, t)
    assert a == b
...
def test_outcome_strict_future_and_maturity():
    ...
    assert {x['horizon_min'] for x in o} == {5, 10, 15, 30, 60}
...
def test_current_forming_endpoint_is_rejected():
    c = candles()
    t = c.index[100]
    now = t + pd.Timedelta(minutes=5, seconds=10)
...
def test_closed_endpoint_is_accepted():
    ...
    o = label_observation(c, t, now=now)
    assert 5 in {x['horizon_min'] for x in o}
...
def test_multihorizon_purge_real_shape():
    ...
    for i in range(500):
        ...
        for h in (5, 10, 15, 30, 60):
            rows.append({'feature_time': t.isoformat(), 'horizon_min': h})
    ...
    assert_clean_boundaries(
        d, v, h,
        pd.Timestamp('2026-01-01T05:00Z'),
        pd.Timestamp('2026-01-01T10:00Z'),
        pd.Timestamp('2026-01-01T12:00Z')
    )
...
def test_grouped_rows_are_one_observation_for_discovery():
    ...
    assert all(len(x['outcomes']) == 5 for x in grouped)
                ...
                'candidates': [{
                    'content': {
                        'parts': [{
                            'text': (
                                '{"hypotheses": ['
                                '{"name":"rvol","direction":"long",'
                                '"horizon_min":15,'
                                '"conditions":[{"feature":"rvol20","op":">=","value":1.0}],'
                                '"rationale":"test"}]}'
                            )
                        }]
                    }
...
def test_evaluation_production_shape():
    ...
    metrics = evaluate_one(rule, _group_rows(_production_rows(800)))
...
def test_integration_maturity_backfill(tmp_path, monkeypatch):
    # The first collector run warms the 65-candle feature requirement.
    # The second run is intentionally far enough ahead that at least one
    # newly-created observation has a fully matured 60-minute outcome.
    ...
    monkeypatch.setattr(runmod, 'OBS_FILE', obs)
    monkeypatch.setattr(runmod, 'OUTCOME_FILE', out)
    monkeypatch.setattr(runmod, 'ERROR_FILE', errors)
    monkeypatch.setattr(runmod, 'CANDLE_DIR', data)
    monkeypatch.setattr(runmod, 'COINS', ('BTC',))
    monkeypatch.setattr(runmod, 'TAKER_FEE_RATE', 0.00075)

    monkeypatch.setattr(research_store, 'STATE_FILE', state)
    ...
    def fake_store(df, symbol, root, retention_days):
        mem[symbol] = pd.concat(
            [mem.get(symbol, pd.DataFrame()), df]
        ).sort_index()
        mem[symbol] = mem[symbol][
            ~mem[symbol].index.duplicated(keep='last')
        ]
    ...
    monkeypatch.setattr(
        runmod,
        'load_candles',
        lambda symbol, root: mem.get(symbol, pd.DataFrame()).copy()
    )

    def fake_fetch(symbol, s, e):
        return c.loc[(c.index >= s) & (c.index <= e)]

    runmod.collect_once(
        pd.Timestamp('2026-01-01T00:50:10Z'),
        fetch_fn=fake_fetch
    )

    # At ~02:10 the newest observations are around 01:59, so the 60-minute
    # horizon is not yet mature. Use 04:10 to make maturity deterministic.
    runmod.collect_once(
        pd.Timestamp('2026-01-01T04:10:10Z'),
        fetch_fn=fake_fetch
    )

    outrows = runmod.read_jsonl(out)
    assert outrows

    horizons = {int(x['horizon_min']) for x in outrows}
    assert {5, 10, 15, 30, 60}.issubset(horizons)
...
def test_delayed_run_backfills_old_unresolved_observation(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        runmod,
        'store_candles',
        lambda df, symbol, root, retention_days:
            store.__setitem__(
                symbol,
                pd.concat([store.get(symbol, pd.DataFrame()), df]).sort_index()
            )
    )
    monkeypatch.setattr(
        runmod,
        'load_candles',
        lambda symbol, root: store.get(symbol, pd.DataFrame()).copy()
    )
    ...
    obs.write_text(
        json.dumps({
            ...
            'symbol': 'BTC',
            'features': feature_snapshot(c, t)
        }) + '\n',
        encoding='utf-8'
    )

    runmod.collect_once(
        c.index[260] + pd.Timedelta(seconds=10),
        fetch_fn=lambda *args, **kwargs: c.iloc[:1]
    )
