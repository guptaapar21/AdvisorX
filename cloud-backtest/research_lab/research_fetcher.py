...
def fetch_1m(symbol: str, start_time, end_time, limit: int = 1000, retries: int = 3) -> pd.DataFrame:
    s = pd.Timestamp(start_time); e = pd.Timestamp(end_time)
    ...
    cur = int(s.timestamp() * 1000); end_ms = int(e.timestamp() * 1000)
    step = 60_000 * limit
    rows = []
    while cur < end_ms:
        params = {
            "pair": f"B-{symbol}_USDT", "interval": "1m",
            "startTime": cur, "endTime": min(cur + step, end_ms), "limit": limit,
        }
        last = None
        ...
        if not data:
            ...
            continue
        rows.extend(data)
        nxt = max(int(x["time"]) for x in data) + 60_000
        cur = max(cur + 60_000, nxt)
        time.sleep(0.15)
    if not rows:
        return pd.DataFrame(columns=["open", "high", "low", "close", "volume"])
