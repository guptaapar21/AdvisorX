...
FEATURE_REGISTRY = (
    "feature_time","feature_schema_version","close",
...
def _coverage(index, t, minutes=60):
    start = t - pd.Timedelta(minutes=minutes)
    idx = pd.DatetimeIndex(index)
    idx = idx[(idx > start) & (idx <= t)]
    expected = minutes
    if not len(idx):
        return 0.0, expected
    diffs = idx.to_series().diff().dropna()
    gap_count = int((diffs != pd.Timedelta(minutes=1)).sum())
    return float(len(idx) / expected), gap_count
...
def feature_snapshot(candles: pd.DataFrame, feature_time) -> dict:
    ...
    if c.empty:
        raise ValueError("No candles at or before feature_time")

    for col in ["open","high","low","close","volume"]:
        c[col] = pd.to_numeric(c[col], errors="coerce")
    c = c.dropna(subset=["open","high","low","close","volume"])
    if c.empty:
        raise ValueError("No valid OHLCV rows at feature_time")
    ...
    coverage, gap_count = _coverage(c.index, t, 60)
    ...
    result = {
        ...
        "return_10m": ret(10), "return_15m": ret(15), "return_30m": ret(30), "return_60m": ret(60),
        "volatility_5m": vol_std(5), "volatility_15m": vol_std(15),
        "volatility_30m": vol_std(30), "volatility_60m": vol_std(60),
        ...
        "rsi14": None if not np.isfinite(rsi) else float(rsi),
        ...
        "efficiency_20": None,
        "bar_coverage_60m": coverage,
        "gap_count_60m": gap_count,
    }
    if len(c) > 20:
        net = abs(float(close.iloc[-1]-close.iloc[-21]))
        gross = float(close.diff().abs().iloc[-20:].sum())
