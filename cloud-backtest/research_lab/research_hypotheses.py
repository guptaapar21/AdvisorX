...
SYSTEM = """You are a neutral market research scientist, not a trader. Use ONLY the DISCOVERY dataset supplied in this request. Do not use validation or holdout rows; those are tested later by Python. You have no authority over AdvisorX. Search for simple consistent conditions associated with small NET returns after fees, reasonable MFE/MAE and low drawdown. Do not assume AdvisorX's current strategy is correct. Consider LONG and SHORT symmetrically. Prefer broad repeatable effects over rare huge winners. Do not invent data. Return JSON only: {hypotheses:[{name,direction,horizon_min,conditions:[{feature,op,value}],rationale}]}. Allowed operators: >, >=, <, <=, between. Hypotheses are research candidates only."""
...
def _sample_by_symbol(disc, per):
    ...
    for sym,vals in buckets.items():
        ...
        if len(vals)<=per:sample.extend(vals)
        else:
...
def discover(api_key, rows, model, memory=None):
    """Compatibility entry point kept for the existing test suite.

    Production runtime uses research_discovery.discover_multi().
    """
...
def evaluate(hypotheses, rows):
    ...
    return [{'hypothesis':h,'validation':evaluate_one(h,val),'holdout':evaluate_one(h,hold)} for h in hypotheses]
