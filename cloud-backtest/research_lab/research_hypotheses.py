...
def _split_grouped(grouped):
    ...
    # Boundary assertions operate at observation level.
...
def discover(api_key, rows, model, memory=None):
    # Accept one or many comma-separated keys. ResearchLab is independent of
    # AdvisorX's key-rotation implementation and rotates locally only on retryable
    # Gemini failures.
    ...
    # Uniform chronological sample per symbol, preserving one row per observation.
    buckets={}
    for r in disc:buckets.setdefault(r.get('symbol','?'),[]).append(r)
    sample=[]
    per=max(1,MAX_DISCOVERY_OBSERVATIONS//max(1,len(buckets)))
    ...
    parsed=_parse(text)
    return {'status':'ok','generated_at':datetime.now(timezone.utc).isoformat(),'model':model,'hypotheses':parsed.get('hypotheses',[]),'discovery_observations':len(disc),'validation_observations':len(val),'holdout_observations':len(hold),'discovery_only':True}
