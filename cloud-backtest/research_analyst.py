...
The analyst consumes the append-only market-behaviour observation/outcome shards,
creates a bounded family of falsifiable candidate rules, validates them on a
chronological development split, and promotes only robust candidates that also
survive a frozen holdout and cost-stress check.
        ...
        for direction in ("bullish", "bearish"):
            for horizon in HORIZONS:
                for label, cond in modifiers:
                    h = {"name": f"{family} | {label}", "event_family": family,
...
def render_live_context(result: dict[str, Any], path: Path) -> None:
    payload = {
        ...
        "validated_formulations": [
            {
                "name": x["hypothesis"]["name"],
                "event_family": x["hypothesis"]["event_family"],
                "direction": x["hypothesis"]["direction"],
                "horizon_min": x["hypothesis"]["horizon_min"],
                "conditions": x["hypothesis"]["conditions"],
...
def main() -> int:
    ...
    result = analyse(rows, args.fee_pct)
