...
def run_structure_backtest(candles, coin, r_target_unused=1.5, trend_lookback=3,
    ...
    for i in range(warmup, len(candles)):
        ...
        if position is None and pending is None and i < len(candles)-1:
            ...
            # IDEA #18F: 3-bar (9 minute) trend-slope confirmation.
            # LONG: EMA9 rising, EMA21 non-falling, VWAP non-falling.
            # SHORT: exact mirror.  We compare the completed signal bar i
            # with i-3 only, so there is no lookahead.
            ema9_prev3 = float(candles["ema9"].iloc[i-3])
            ema21_prev3 = float(candles["ema21"].iloc[i-3])
            vwap_prev3 = float(candles["vwap"].iloc[i-3])
            long_trend = (ema9 > ema21 and ema9 > ema9_prev3 and
                          ema21 >= ema21_prev3 and vwap >= vwap_prev3 and
                          float(row["close"]) > vwap)
            short_trend = (ema9 < ema21 and ema9 < ema9_prev3 and
                           ema21 <= ema21_prev3 and vwap <= vwap_prev3 and
                           float(row["close"]) < vwap)

            # IDEA #18F rejection structure:
            # LONG requires an immediately preceding FALLING (bearish) candle,
            # followed by a bullish rejection candle which does NOT break that
            # falling candle's low.  SHORT is the exact mirror.
            prev = candles.iloc[i-1]
            prev_falling = float(prev["close"]) < float(prev["open"])
            prev_rising = float(prev["close"]) > float(prev["open"])

            zone_lo, zone_hi = min(ema9, ema21), max(ema9, ema21)
            touched_zone = float(row["low"]) <= zone_hi and float(row["high"]) >= zone_lo

            bullish_rejection = (touched_zone and
                                  float(row["close"]) > float(row["open"]) and
                                  float(row["close"]) >= ema9 and
                                  prev_falling and
                                  float(row["low"]) >= float(prev["low"]))
            bearish_rejection = (touched_zone and
                                  float(row["close"]) < float(row["open"]) and
                                  float(row["close"]) <= ema9 and
                                  prev_rising and
            ...
            if direction:
                ...
                if not adx_confirmation_ok:
                    continue
                # IDEA #18F stop is the extreme of the preceding pullback candle,
                # NOT the old 3-bar swing stop. Target remains the prior local
                # structure high/low (10 completed bars, excluding signal bar).
...
def main():
    ...
    if mode == "adx-quality-sweep":
        ...
        print(f"Running Idea #18F rejection-structure ADX quality sweep: {len(tasks)} variants")
        print(f"Fee assumption: {args.fee_rate*100:.4f}% per side ({2*args.fee_rate*100:.4f}% round trip before other costs)")
        print("FIXED: targetLB=10, structural RR>=1.0, falling/rising-candle SL, "
              "min-stop>=0.25%, 6m momentum checkpoint; EMA9/EMA21/VWAP 3-bar slope gate")
        ...
        print("\n=== IDEA #18F: REJECTION-STRUCTURE + EMA/VWAP SLOPE + ADX SWEEP ===")
