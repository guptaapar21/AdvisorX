...
BREAKOUT_MAX_BARS = int(os.environ.get("BREAKOUT_MAX_BARS", "8"))
...
def evaluate_entry_quality(signal: Dict[str, Any], snapshot: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    s3, s15, s1h = ms.get("3m") or {}, ms.get("15m") or {}, ms.get("1h") or {}
    ...
    if regime in {"TREND_UP", "TREND_DOWN", "EXHAUSTION_OR_TRANSITION"}:
        ...
        if s15.get("structure_bias") != wanted:
            return False, "15m structure is not aligned with proposed direction"
    ...
    if regime in {"BREAKOUT_TRANSITION", "BREAKDOWN_TRANSITION"}:
        ...
        if str(latest.get("direction") or "").lower() != expected:
        ...
        if _finite(ext) and abs(float(ext)) > BREAKOUT_MAX_EXTENSION_ATR:
            return False, f"breakout entry is extended {float(ext):.2f} ATR"
    ...
    if cq == "late_extended":
        ext = cont.get("extension_atr_from_break")
        if _finite(ext) and abs(float(ext)) > BREAKOUT_MAX_EXTENSION_ATR:
            return False, "continuation is late and extended beyond the allowed ATR"

    if regime in {"TREND_UP", "TREND_DOWN", "BREAKOUT_TRANSITION", "BREAKDOWN_TRANSITION"}:
        if (s3.get("exhaustion_flags") or []) and (s3.get("failed_breaks") or []):
            return False, "exhaustion and failed-break evidence conflict with continuation"
