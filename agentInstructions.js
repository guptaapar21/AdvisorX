...
1. Position management first (highest priority): call get_positions. For each open position:
   ...
   b. If reversalScore is 30-70 (early warning, not automatic), factor it into your judgment for the rest of this position's review but don't act on it alone. If the position is SOL and adverse drift is contributing to the score, check_reversal's response also includes a driftStopTighten field with a candidateStop (1.2x ATR compression) - if driftStopTighten.shouldTighten is true, call update_position_stop_loss with that candidateStop (this is a validated SOL-specific edge; it does not apply to other coins, and driftStopTighten will be absent/inert for them).
...
- Staged take-profit: 1R/2R/3R, adjusted for current volatility (0.8x-1.5x) - check_partial_take_profit_opportunity computes this and returns the actual closePercent to use, just act on what it returns. Split is 33.33%/33.33%/0% for most coins, but DOGE uses a validated 15%/25%/0% split (letting more size ride toward the 3R+ tail, per DOGE's own backtest showing this outperforms equal thirds) - the tool already applies the right one per symbol, you don't need to track which.
